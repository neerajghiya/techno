...
public class OnlineLeadsCronService {
	public static void main(String[] args) throws IOException {
		OnlineLeadsCronService cronService = new OnlineLeadsCronService();
	...
	private void readAndRunDuplicateFailureFiles() throws IOException {
		...
		String failureFilePath = "/StarEAI/DPLogs/OnlineLeads/failed/pushLead2EAI";
	...
	private static List readCSVFile() throws IOException {
		...
		Calendar cal = Calendar.getInstance();
		try {
			File file = new File(csvFilePath + "/" + dateFormat.format(cal.getTime()) + "/onlineLeads.csv");
			if (file.exists()) {
				System.out.println("File exists");
	...
	private static void updateFile(String lead, boolean appendComma) throws IOException{
		...
		try {
			FileWriter file = new FileWriter(csvFilePath + "/"
					+ dateFormat.format(cal.getTime()) + "/onlineLeads.csv", true);
			if (appendComma) {
				file.append(",");
			}
			file.append(lead);
